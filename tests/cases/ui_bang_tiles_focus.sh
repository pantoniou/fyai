...
FYAI_PTY_AFTER="send:!sh -c 'printf SECOND; sleep 5'|wait:SECOND|raw:14|wait:focused|raw:14|raw:14|send:/status|wait:Usage / total" \
...
if not any("Usage / total" in row for frame in shown for row in frame):
    raise SystemExit("Ctrl-T did not round-trip back to the prompt")
PYEOF
