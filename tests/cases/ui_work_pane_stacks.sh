...
# Verify the work pane falls back to a stack when a tile cannot keep the
# configured width. ui_agents_parallel runs the same delegation on the same
# hundred-column terminal and gets two tiles side by side; the only
# difference here is the width a tile is required to keep, so what this case
# proves is that the setting is read and that a terminal too narrow for it
# shows one screen above another instead of two unreadable columns.
set -eu
...
for row_a, row_b, row in frames:
    ...
    # Stacked, and each still has the full width to itself: with one column
    # there is nothing for a column rule to divide.
    if "\u2503" in row[20:]:
