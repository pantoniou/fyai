...
case "$MODE" in
chat-completions) MODE_ARGS=(--set api=chat-completions --set api_url="$MOCK_URL/v1/chat/completions") ;;
responses)        MODE_ARGS=(--set api=responses --set api_url="$MOCK_URL/v1/responses") ;;
messages)         MODE_ARGS=(--set api=messages --set api_url="$MOCK_URL/v1/messages") ;;
