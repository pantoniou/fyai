...
# from the process that watches SIGWINCH. The window changes once the program
# says that its trap is set.
set -eu
...
FYAI_PTY_NEEDLE="TRAP-SET" \
"$PYTHON" "$TESTS_DIR/pty_resize.py" "$TEST_DIR/pty.out" \
    "$FYAI_BIN" tool shell \
    '{"command":"trap \"stty size\" WINCH; echo TRAP\"\"-SET; sleep 5 & p=$!; wait $p","tty":true,"timeout":10000}' \
