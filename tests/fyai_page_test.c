...
static int page_grid_places_the_tiles_run(void)
{
	...
	g.rows = 1;
	g.cols = 2;
	rc = fyai_page_grid(&g, cells, 2, 0, " | ", 3, &src, &rows);
...
static int page_grid_shares_the_height_run(void)
{
	...
	g.rows = 2;
	g.cols = 1;
	rc = fyai_page_grid(&g, cells, 2, 12, "", 0, &src, &rows);
	...
	rc = fyai_page_grid(&g, cells, 2, 0, "", 0, &src, &rows);
...
static int page_grid_fits_and_spans_run(void)
{
	...
	g.col_size[0] = 20;
	rc = fyai_page_grid(&g, cells, 3, 10, "", 0, &src, &rows);
	...
	src = (struct response_buffer){0};
	g.rows = 1;
	rc = fyai_page_grid(&g, cells, 3, 0, "", 0, &src, &rows);
				  ...
				  struct fyai_page_keys *keys)
{
	...
	if (fyai_page_transcribe(page_yaml(gb, doc), page_yaml(gb, state),
