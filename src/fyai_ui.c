	...
	if (want && !ui->page) {
		if (!fyai_page_supported()) {
			fyai_warning(ctx, "display/renderer page needs a libfytimui "
				     "and a libfymd4c with page support; "
...
static void ui_page_update(struct fyai_ui *ui)
{
	...
	sep_cols = ctx->cfg->tile_sep ?
		   (int)fymd_str_width(ctx->cfg->tile_sep,
				       strlen(ctx->cfg->tile_sep)) : 0;
	if (!pane || fyai_workpane_page_grid(ctx->workpane, 0,
	...
	ui->pane_grid.len = 0;
	if (st.pane_rows > 0 &&
	    !fyai_workpane_page_grid(ctx->workpane, st.pane_rows,
				     ctx->cfg->tile_sep, sep_cols,
				     &ui->pane_grid, NULL))
		st.pane_source = ui->pane_grid.data;
	if (fyai_page_publish(ui->page, ui->ft, &st, cols, rows)) {
		fyai_page_destroy(ui->page);
		ui->page = NULL;
		fytim_page_clear(ui->ft);
