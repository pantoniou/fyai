...
struct fyai_terminal_view {
	...
	int cols;
	/*
	 * The painted reading. A renderer repaints the rows that changed, thus
	 * the damage of the screen layer is kept as one row range. The cursor
	 * is recorded where the program put it, which is not where the text
	 * ends: a program can leave it anywhere on the screen.
	 */
	int damage_first;
	int damage_last;
	int cursor_row;
...
/*
 * A program hides and shows the cursor around a redraw. That is presentation
 * and not content, thus only a renderer uses it; every other reading of the
 * view ignores it.
 */
...
/*
 * There is no moverect callback, and that is deliberate. Without one the
 * screen layer damages the region that a scroll moved, so a scroll needs no
 * case of its own.
 */
						     ...
						     size_t max_bytes)
{
	...
	/*
	 * A renderer paints whole rows, thus cell damage would only make the
