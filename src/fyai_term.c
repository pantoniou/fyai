 ...
 * display is open, so it cannot give the size, and the display samples the
 * terminal at each frame. The window gives the width.
 *
 * The rows have two sources, and they must not follow each other. The band
 * grants rows to the surface, and the program keeps the rows that the surface
 * does not use.
 */
...
/*
 * Ask the program to stop, and make sure that it stops. Send SIGHUP first,
 * because that is the end of a terminal, and an interactive shell stops on it.
 * Such a shell ignores SIGTERM, which is what ^\ q meets in normal use. Send
 * SIGTERM next for a program that reads it, and SIGKILL after the grace time.
 */
...
/*
 * One key. The prefix is the only key fyai keeps: everything else is the
 * program's, including ^C, which its own line discipline turns into a signal.
 */
static void term_key(struct fyai_term *t, char c)
{
	...
	if (t->prefix) {
		...
		case 'Q':
			/*
			 * Leaving is asked for here and finished when the
			 * program has gone. Ending the loop now would stop
			 * the timer that insists, and the wait for a child
			 * that ignored the asking would never return.
			 */
			t->quitting = true;
			term_cancel(t);
			return;
...
static enum fyai_event_action term_child(const struct fyai_event *ev)
{
	 ...
	 * can hold the terminal open, thus the end is the child being reaped
...
int fyai_term_verb(struct fyai_ctx *ctx)
{
	...
	/*
	 * The program gets the width of the terminal, and the rows that stay after
	 * the prompt and the chrome. A run with no terminal, such as a test, takes
	 * the default size.
	 */
	...
	t.view = fyai_terminal_view_create(t.rows, t.cols, 0);
	...
	/*
	 * No sandbox: the person at the keyboard is the one running the shell,
	 * and confinement is for a command the model asked for.
	 */
	...
	while (!t.done) {
		term_frame(&t);
		/*
		 * An interrupt from outside this terminal ends the program. A
		 * ^C typed here is a key and reaches the program instead.
		 */
		...
		/*
		 * A terminal is a live view. The window can change size while no byte
		 * arrives from the program and no key is pressed. The wait is therefore
		 * bounded, and each turn of the loop samples the size.
		 */
	...
	/*
	 * Nothing is left to draw, but the program can still run. The loop keeps
	 * running for it, because the timer that turns the request into a SIGKILL
	 * lives on that loop. A plain wait here would wait forever for a program that
	 * ignored the request.
	 */
	term_cancel(&t);
