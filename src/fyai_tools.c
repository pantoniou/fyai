...
/*
 * True when a shell call opens a named session. A name makes the shell
 * addressable. The shell keeps its own process and terminal until it closes.
 * `shell_input`, `shell_output` and `shell_close` use that name.
 */
static bool fyai_shell_session_call(struct fyai_ctx *ctx, fy_generic tool_call)
{
	fy_generic args, name;
	...
	return fy_is_string(name) && !fy_empty(name);
}
...
void fyai_tool_progress_emit(struct fyai_ctx *ctx, const char *data, size_t len)
{
	...
	if (!ctx || !ctx->tool_rpc || !len)
		return;
	/*
	 * A sub-agent with a terminal of its own has already shown this: the
	 * parent reads that terminal. Sending it again would be the same text
	 * twice, on a channel that exists for a display that is gone.
	 */
...
/*
 * Give the child the terminal that it was spawned with. The child called
 * setsid(), so it has no controlling terminal. This call makes @slave its
 * controlling terminal. The program can then read the size of the terminal and
 * receive SIGWINCH.
 */
...
static int fyai_tool_child_fds(int req_fd, int rsp_fd)
{
	...
	/*
	 * Nothing in the child reads standard input. Do not leave standard input on
	 * the terminal of the user. A child with a terminal of its own keeps that
	 * terminal on each standard descriptor, because a program can ask about any
	 * of them.
	 */
...
/*
 * Open a session in this child. The call is answered as soon as the terminal
 * exists, because the model reads when it wants to and the parent renders
 * continuously from `shell/output`. This child then serves the session until
 * the program ends, the parent closes it, or its limit expires.
 */
				    ...
				    struct jsonrpc_conn *conn)
{
	...
	jsonrpc_conn_respond(conn, tc->id,
		fy_gb_mapping(gb, "result",
			      fy_gb_mapping(gb, "session", started,
	...
	/*
	 * Serve the session until its program ends. A signal reaches this child and
	 * not the program, because the program leads its own group. Stop the program
	 * from here, or the parent waits for a child that does not stop.
	 */
	el = fyai_ctx_loop(ctx);
	while (el && !fyai_terminal_relay_done(tc->relay)) {
...
/*
 * A named terminal session. The process that drives it is the job. The view
 * holds the state of the terminal, and it stays after that process ends, so a
 * caller can still read the last screen and the log. A name is unique among
 * the live sessions of this branch.
 */
struct fyai_shell_session {
	...
	/*
	 * The session is the display, and not the call that opened it. It has one
	 * surface. The surface is live while the program runs, and it is marked
	 * running until the program stops. The calls that write to the session and
	 * read it show nothing, because their effect is on this screen.
	 */
...
/*
 * The terminal that a sub-agent renders into. It is as wide as the display,
 * less the margin. It is only as tall as the band shows, because the tool
 * result carries the full report.
 */
...
static enum fyai_event_action fyai_shell_session_idle(const struct fyai_event *ev)
{
			  ...
			  int cols, size_t max_bytes)
{
	...
	sess->view = fyai_terminal_view_create(rows, cols, max_bytes);
	if (!sess->name || !sess->command || !sess->branch || !sess->view) {
	...
	if (sess->surface) {
		 ...
		 * were, and the session shows nothing until the program writes.
		 */
				      ...
				      bool *okp)
{
	...
	*okp = true;
	...
	/*
	 * A session that was asked to close ends as it was told to. The signal that
	 * closed it is not a failure of the program.
	 */
	if (sess->closing)
		return NULL;
	if (sess->signal) {
...
/*
 * The session is over: its screen is committed, which is what puts it in the
 * transcript. Until this point the call that opened it has shown nothing but
 * a screen marked running, because that is what it was.
 */
...
/*
 * End every session. A session lives for one invocation: its process cannot
 * outlive it, and nothing later could address it. @force does not wait, which
 * is what an interrupt asks for.
 */
						    ...
						    struct fyai_terminal_region *region)
{
	...
	if (fy_equal(view, "region")) {
		...
		region->row = (int)fy_get(r, "row", 0LL);
		region->col = (int)fy_get(r, "col", 0LL);
		region->rows = (int)fy_get(r, "rows", 0LL);
		region->cols = (int)fy_get(r, "cols", 0LL);
				       ...
				       const struct fyai_terminal_region *region)
{
	...
	response_buffer_append(&buf, bounded ? bounded : text);
	free(bounded);
	free(text);
...
fyai_shell_session_of(struct fyai_ctx *ctx, fy_generic args, char **errp)
{
	...
	*errp = NULL;
	 ...
	 * nothing, thus say so rather than let the model wait for output.
	 */
				   ...
				   bool *okp)
{
	...
	*okp = false;
	...
	fyai_shell_session_idle_arm(sess);
	*okp = true;
	return fyai_shell_session_result(ctx, sess, args,
				   ...
				   bool *okp)
{
	...
	*okp = true;
	if (sess->exited)
		return strdup(fy_sprintfa("[shell '%s' ended: status %d]",
					  sess->name, sess->signal ?
					  128 + sess->signal : sess->exit_code));
	return strdup(fy_sprintfa("[shell '%s' was asked to end]", sess->name));
}
				      ...
				      fy_generic tool_call)
{
	...
	name = fyai_tool_call_name(ctx, tool_call);
	/*
	 * A session tool reaches the terminal state that the parent holds, so it runs
	 * there and not in a forked child. No session tool waits for a program, so
	 * nothing is serialised for long.
	 *
	 * `time` and `wait` run there for the same reason. A named wait arms a timer
	 * that must outlive the call, and only the loop of the parent lives that
	 * long. A forked child would take the timer with it.
	 */
...
/*
 * Give the sub-agent the terminal that it is drawn in. The band grants a width
 * and a number of rows. A program with another size wraps where nothing is
 * shown. The parent holds this master, so the parent sets the size and the
...
/*
 * Give a sub-agent's terminal somewhere to be shown. Its screen is a surface,
 * exactly as a shell session's is: one way of showing a running thing, and no
 * second one for sub-agents.
 */
				...
				struct fyai_tool_job *job)
{
	...
	job->view = fyai_terminal_view_create(job->pty_rows, job->pty_cols, 0);
	if (!job->view)
		return -1;
	el = fyai_ctx_loop(ctx);
	...
	if (job->surface) {
		...
		(void)fyai_ui_surface_set_margin(job->surface,
						 ctx->cfg->session_margin);
		/*
		 * Paint the empty screen immediately, as a session does. Without it the rows
		 * that the band took stay as they were, and the sub-agent shows nothing
		 * until it writes.
		 */
...
static void fyai_tool_job_update_done(struct fyai_tool_job *job)
{
	...
	/*
	 * A session answers as soon as its terminal exists and then keeps
	 * running, thus the answer completes the call and the process is
	 * released later. Every other job ends with its process.
	 */
 ...
 *
 * @pty gives the child a terminal of its own for its standard descriptors. A
 * delegated sub-agent asks for one. It then renders as this program renders,
 * and the parent interprets that terminal and shows it. There is one way to
 * show work that runs. The control channel does not change: it stays on
 * descriptors 3 and 4.
 */
			       ...
			       struct fyai_tool_job *job, bool pty)
{
	...
	if (!pid) {			/* child */
		...
		if (master >= 0)
			close(master);
		/*
		 * The terminal before the descriptors are arranged: it becomes
					    ...
					    fy_generic tool_call)
{
	...
	/*
	 * Reserve the session name before the spawn, as a sub-agent branch is
	 * reserved. A name is unique among the live sessions of this branch. A name
	 * in use is refused, so that the model selects another name.
	 */
	...
	if (fy_any_equal(name, "shell", "agent") &&
		...
		/*
		 * A sub-agent has a terminal of its own and renders to it, so
		 * its screen is shown as any other screen is. Only a shell
		 * still streams its output into a band here.
		 */
		if (job->agent) {
				 ...
				 struct fyai_tool_job *job, bool *okp)
{
	...
	/*
	 * A session is complete when its terminal exists; its process keeps
	 * running and belongs to the session from here on. The group drops its
	 * reference, thus the job must not be reaped or released here.
	 */
	if (job->session) {
		fyai_tool_job_live_close(job);
		fyai_tool_job_drop(&job->deadline);
		...
		*okp = job->result_ok && !job->failed;
		if (!*okp) {
			/*
			 * Quote the cause the child recorded. The model reads
			 * the reason here, and the user still receives the
			 * same diagnostic: rendering it does not consume it.
			 */
			char *cause = fyai_diag_string(fyai_ctx_diag(ctx));
			fy_generic out = fy_value(ctx->transient_gb,
				fy_sprintfa("tool error: the terminal session "
		...
		return fy_value(ctx->transient_gb,
			fy_sprintfa("[terminal session '%s' started: %s]\n"
				    "Read it with shell_output, drive it with "
				    "shell_input, end it with shell_close.",
				    job->session->name, job->session->command));
	}
	fyai_tool_job_live_close(job);
	fyai_tool_job_close_channel(job);
	fyai_tool_job_drop(&job->deadline);
